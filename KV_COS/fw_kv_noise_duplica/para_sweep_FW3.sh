#!/usr/bin/env bash
set -euo pipefail

# ==============================
# 基本設定
# ==============================
EPOCHS=${EPOCHS:-150}
TRAIN_STEPS=${TRAIN_STEPS:-10000}
VAL_STEPS=${VAL_STEPS:-2000}
BATCH=${BATCH:-32}

# ==============================
# モデル設定
# ==============================
CORE_TYPE="fw"

# ==============================
# 次元設定
# ==============================
DG=${DG:-30}
DH=${DH:-60}
NUM_CLASSES=${NUM_CLASSES:-8}

# ==============================
# 最適化設定
# ==============================
LR=${LR:-3e-3}
WD=${WD:-5e-4}
CLIP=${CLIP:-0.7}

# ==============================
# タスク設定
# ==============================
T_BIND=${T_BIND:-12}
DELTA_WAIT=${DELTA_WAIT:-2}
BETA=${BETA:-0.0}
DUPLICATE=${DUPLICATE:-4}      # ★★ 復活 ★★

# ★★ NOISE（bind/query）固定 ★★
SIGMA=${SIGMA:-1.0}

# ==============================
# Sweep 対象（λ × η）
# ==============================
LAMBDA_LIST=${LAMBDA_LIST:-"0.65 0.80 0.95"}
ETA_LIST=${ETA_LIST:-"0.30 0.50 0.70"}

S_LIST=${S_LIST:-"3"}
SEEDS=${SEEDS:-"0"}

# ==============================
# 実行環境
# ==============================
GPU=${GPU:-1}
PYTHON_BIN=${PYTHON_BIN:-python3}

# ==============================
# 出力ディレクトリ
# ==============================
OUT_DIR=${OUT_DIR:-"results_recon_noise_fixed_sweep_FW3"}

if [[ -d "${OUT_DIR}" ]]; then
  BACKUP="${OUT_DIR}_backup_$(date +"%Y%m%d_%H%M%S")"
  echo "[WARN] Moving old results → ${BACKUP}"
  mv "${OUT_DIR}" "${BACKUP}"
fi
mkdir -p "${OUT_DIR}"

GPU_FLAG=""
if [[ "${GPU}" == "1" ]] && command -v nvidia-smi >/dev/null 2>&1; then
  GPU_FLAG="--gpu"
fi

# ==============================
# Config Log
# ==============================
CONFIG_FILE="${OUT_DIR}/run_config.txt"
{
echo "[INFO] Lambda × Eta Sweep (noise fixed)"
echo "DG=${DG}, DH=${DH}, NUM_CLASSES=${NUM_CLASSES}"
echo "LR=${LR}, WD=${WD}"
echo "T_BIND=${T_BIND}, DELTA_WAIT=${DELTA_WAIT}, BETA=${BETA}, DUPLICATE=${DUPLICATE}"
echo "NOISE=${SIGMA}"
echo "LAMBDA_LIST=${LAMBDA_LIST}"
echo "ETA_LIST=${ETA_LIST}"
echo "S_LIST=${S_LIST}"
echo "SEEDS=${SEEDS}"
} > "${CONFIG_FILE}"

# ==============================
# 1条件だけ実験する関数
# ==============================
run_one () {
  local LAM="$1"
  local ETA="$2"
  local S="$3"
  local SEED="$4"

  local OUT_CSV="${OUT_DIR}/lam${LAM}_eta${ETA}_S${S}_seed${SEED}.csv"

  echo "=== RUN λ=${LAM} η=${ETA} S=${S} seed=${SEED} σ=${SIGMA} dup=${DUPLICATE} ==="

  ${PYTHON_BIN} run.py \
    --epochs "${EPOCHS}" \
    --train_steps "${TRAIN_STEPS}" \
    --val_steps "${VAL_STEPS}" \
    --batch_size "${BATCH}" \
    --d_g "${DG}" \
    --d_h "${DH}" \
    --num_classes "${NUM_CLASSES}" \
    --T_bind "${T_BIND}" \
    --duplicate "${DUPLICATE}" \
    --S "${S}" \
    --beta "${BETA}" \
    --delta_wait "${DELTA_WAIT}" \
    --lambda_decay "${LAM}" \
    --eta "${ETA}" \
    --seed "${SEED}" \
    --lr "${LR}" \
    --weight_decay "${WD}" \
    --grad_clip "${CLIP}" \
    --core_type "${CORE_TYPE}" \
    --bind_noise_std "${SIGMA}" \
    --query_noise_std "${SIGMA}" \
    --use_fw --use_ln ${GPU_FLAG} \
    --out_csv "${OUT_CSV}"
}

# ==============================
# Sweep Loop（λ × η × S × seed）
# ==============================
echo "==== Running Sweep λ × η × S × seed (σ=${SIGMA}, dup=${DUPLICATE}) ===="

for S in ${S_LIST}; do
  for LAM in ${LAMBDA_LIST}; do
    for ETA in ${ETA_LIST}; do
      for SEED in ${SEEDS}; do
        run_one "$LAM" "$ETA" "$S" "$SEED"
      done
    done
  done
done

echo "============================================"
echo "  All experiments completed!"
echo "  Results saved in: ${OUT_DIR}/"
echo "============================================"
