...
def process_model(sloop_csv, amat_csv, out_dir):
    ...
    plt.savefig(os.path.join(out_dir, "cos_summary.eps"))
    plt.close()
    ...
    # まず全体の最大 ratio を集計
    all_ratio_vals = []
    for _, s_list, ratio_base in ratio_plots:
        all_ratio_vals += ratio_base
    ymax_ratio = max(all_ratio_vals)
    ...
    plt.savefig(os.path.join(out_dir, "ratio_summary.eps"))
    plt.close()
    ...
    # 全体の最大値を集計
    all_vals = []
    for _, s_list, norm_h, norm_Ah in norm_plots:
    ...
    plt.savefig(os.path.join(out_dir, "norm_summary.eps"))
    plt.close()
...
def main():
    ...
    for s_csv in sloop_list:
        core, S, eta, lam, seed = parse_filename(s_csv)

        # Amat_kv_xxx を特定
        pattern = f"Amat_kv_{core}_S{S}_eta{eta}_lam{lam}_seed{seed}.csv"
        amat_csv = None
        for a in amat_list:
            if pattern in a:
                amat_csv = a
