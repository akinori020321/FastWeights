...
class KVDataset:
    ...
    def sample_batch(self, B: int) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        # # ============================================================
        # # 修正：Bind クラス列を 1 つだけ作成し、バッチ全体で共有する
        # # ============================================================
        # if T_bind <= C:
        #     base_labels = self.rng.choice(C, size=T_bind, replace=False)
        # else:
        #     base = self.rng.permutation(C)
        #     rest = self.rng.choice(C, size=(T_bind - C), replace=True)
        #     base_labels = np.concatenate([base, rest])

        # y_bind = np.tile(base_labels[None, :], (B, 1))  # (B, T_bind)
        # v = self.mu[y_bind]
