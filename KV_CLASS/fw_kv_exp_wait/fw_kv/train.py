...
from torch import amp  # 新しいAMP API

from .utils import spectral_norm_batch
...
# エポック単位の学習・評価ループ
# ======================================================
def run_epoch_seq(model, head, data, opt=None, cfg=None, num_classes=10, T_bind=5, S=1):
    """
    1エポック分の学習または評価を実行する。

    Args:
        model, head: モデル本体と出力層
        data: データセット (KVDataset)
        opt: Optimizer (Noneなら評価モード)
        cfg: TrainCfg 設定
        S: Query反復回数 (Fast Weights内)
    """
        ...
        for step in range(steps):
            ...
            with amp.autocast("cuda", enabled=cfg.device.startswith("cuda")):
                loss, acc, stats = model.forward_episode(
                    z_seq, y, head=head, S=S, compute_stats=compute_stats
                )
