...
# 出力ディレクトリ（安全設計）
# ==========================================
OUT_DIR=${OUT_DIR:-"results_wait_tanh"}
...
{
  echo "[INFO] RNN Wait Sweep Configuration"
...
for S_FIXED in ${S_LIST}; do
  for DELTA_WAIT in ${DELTA_WAIT_LIST}; do
    for SEED in ${SEEDS}; do
      ...
      OUT_CSV="${OUT_DIR}/rnn_sigma${SIGMA}_wait${DELTA_WAIT}_S${S_FIXED}_seed${SEED}.csv"

      echo "------------------------------------------"
      echo "[RUN] RNN | σ=${SIGMA} | Δ_wait=${DELTA_WAIT} | S=${S_FIXED} | seed=${SEED}"
      echo "------------------------------------------"
...
echo "=========================================="
echo "✅ All RNN σ=0 Noise + Wait sweep experiments completed."
